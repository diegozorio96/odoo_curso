...
class Course(models.Model):
    ...
    _sql_constraints = [
        ('name_description_check','CHECK(name != description)', "El titulo del curso no debe ser igual a la descripcion "),

        ('name_unique','UNIQUE(name)',"El nombre del  curso debe ser único"),
    ]
...
class Session(models.Model):
    ...
    instructor_id = fields.Many2one('res.partner', string="Instructor", domain=['|',('instructor','=', True), ('category_id.name','ilike',"Profesor")])
    ...
    attendee_count = fields.Integer(string="Cantidad Asientos", compute="_get_attendees_count", store= True)
        ...
        if self.seats < 0:
            return{
                'warning':{
                    ...
                    'message':"La cantidad de asientos disponibles no puede ser negativa",
                },
            }
    ...
    def _check_instructor_not_in_attendees(self):
        for r in self:
            if r.instructor_id and r.instructor_id in r.attendee_ids:
                raise exceptions.ValidationError("El instructor de una sesión no puede ser asistente")
